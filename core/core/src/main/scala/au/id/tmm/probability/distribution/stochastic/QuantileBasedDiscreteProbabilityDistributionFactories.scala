...
import au.id.tmm.probability.distribution.stochastic.ProbabilityDistribution.always
...
trait QuantileBasedDiscreteProbabilityDistributionFactories {
  ...
  def fromQuantileFunction[A](quantileFunction: DoubleProbability => A): ProbabilityDistribution[A] =
